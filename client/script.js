...
var createCheckoutSession = async function () {
  ...
  var result = await fetch("/create-checkout-session", {
    method: "POST",
    headers: {
...
fetch("/config")
  ...
  .then(function (json) {
    ...
    if (setupAccounts(json)) {
      ...
      document.querySelector("#submit").addEventListener("click", async function (evt) {
        const {data, account} = await createCheckoutSession();
